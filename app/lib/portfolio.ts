...
export function resolvePaperSnapshot(input:{universe:string[];top4:string[];previousTop4:string[];entryPrices:Record<string,number>;exitPrices:Record<string,number>;funding:Record<string,number>;roundTripCostPct:number}){
 ...
 // Equal-weight benchmark remains fully invested; unchanged weekly membership has no turnover charge.
 const benchmarkReturnPct=benchmarkGross+benchmarkFunding;
