import test from "node:test";
import assert from "node:assert/strict";
import { mkdtemp, readFile, rm, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join } from "node:path";
import {
  appendDecision,
  mutationIsSameOrigin,
  readDecisions,
  type DecisionInput,
} from "./decision-journal";
import { resetHistoryCache, type SignalRecord } from "./store";

/** What the server itself recorded when the candle closed. The only trusted evidence. */
const record: SignalRecord = {
  key: "BTC-1725000000000",
  coin: "BTC",
  sig: "LONG",
  score: 5,
  mode: "TREND",
  signal_closed_at: 1725000000000,
  recorded_at: 1725000001000,
  close: 60000,
  entry: 60000,
  stop: 59000,
  tp1: 61000,
  tp2: 62000,
  risk_pct: 1.67,
  rsi: 42,
  trend_1h: "BULL",
  atr_pct: 1.2,
  reasons: ["harga>EMA50"],
};

async function withHistory(rows: SignalRecord[], work: (dir: string) => Promise<void>) {
  const dir = await mkdtemp(join(tmpdir(), "decision-journal-"));
  const previous = process.env.SCREENER_DATA_DIR;
  process.env.SCREENER_DATA_DIR = dir;
  resetHistoryCache();
  await writeFile(join(dir, "signals.jsonl"), rows.map((r) => JSON.stringify(r)).join("\n") + "\n");
  try { await work(dir); }
  finally {
    if (previous === undefined) delete process.env.SCREENER_DATA_DIR;
    else process.env.SCREENER_DATA_DIR = previous;
    resetHistoryCache();
    await rm(dir, { recursive: true, force: true });
  }
}

const paper = (actual_entry: number, actual_risk_pct: number): DecisionInput =>
  ({ signal_key: record.key, action: "PAPER", actual_entry, actual_risk_pct });
const watch = (): DecisionInput => ({ signal_key: record.key, action: "WATCH" });
const skip = (skip_reason: "RISK_TOO_HIGH" | "LATE" | "STRUCTURE_UNCLEAR" | "EVENT_RISK" | "OTHER", note?: string): DecisionInput =>
  ({ signal_key: record.key, action: "SKIP", skip_reason, note });

test("evidence is copied from server history, never from the request body", async () => {
  await withHistory([record], async (dir) => {
    const forged = {
      ...paper(59950, 0.75),
      // A hostile or stale client cannot rewrite the setup it is judging.
      signal: { ...record, score: 99, tp1: 999_999, close: 1, reasons: ["dibuat-buat"] },
    } as unknown as DecisionInput;
    const saved = await appendDecision(forged, 1725001234567);
    assert.equal(saved.signal.score, 5);
    assert.equal(saved.signal.tp1, 61000);
    assert.deepEqual(saved.signal.reasons, ["harga>EMA50"]);
    // Close is the closed-candle close the server stored, not a live mark.
    assert.equal(saved.signal.close, 60000);
    assert.equal(saved.signal.entry, 60000);
    assert.equal(saved.signal.stop, 59000);

    const disk = await readFile(join(dir, "signal-decisions.jsonl"), "utf8");
    assert.deepEqual(JSON.parse(disk.trim()), saved);
  });
});

test("a decision about a signal the server never recorded is refused", async () => {
  await withHistory([record], async () => {
    await assert.rejects(
      () => appendDecision({ ...watch(), signal_key: "ETH-1725000000000" }, 1),
      /tidak ditemukan/i,
    );
  });
});

test("PAPER records the actual entry and risk the user chose", async () => {
  await withHistory([record], async () => {
    const saved = await appendDecision(paper(59950, 0.75), 1725001234567);
    assert.equal(saved.decided_at, 1725001234567);
    assert.equal(saved.action, "PAPER");
    if (saved.action !== "PAPER") throw new Error("expected PAPER");
    assert.equal(saved.actual_entry, 59950);
    assert.equal(saved.actual_risk_pct, 0.75);
    assert.deepEqual(await readDecisions(), [saved]);
  });
});

test("PANTAU records no invented fill or trade outcome", async () => {
  await withHistory([record], async () => {
    const saved = await appendDecision(watch(), 1725001234567);
    assert.equal(saved.action, "WATCH");
    assert.equal("actual_entry" in saved, false);
    assert.equal("outcome" in saved, false);
  });
});

test("LEWATI requires a bounded reason choice and preserves a free note", async () => {
  await withHistory([record], async () => {
    await assert.rejects(() => appendDecision({ signal_key: record.key, action: "SKIP" } as DecisionInput, 1), /alasan/i);
    const saved = await appendDecision(skip("RISK_TOO_HIGH", "Dekat resistance harian"), 2);
    if (saved.action !== "SKIP") throw new Error("expected SKIP");
    assert.equal(saved.skip_reason, "RISK_TOO_HIGH");
    assert.equal(saved.note, "Dekat resistance harian");
  });
});

test("invalid PAPER numbers are rejected", async () => {
  await withHistory([record], async () => {
    await assert.rejects(() => appendDecision(paper(0, -1), 1), /entry/i);
    await assert.rejects(() => appendDecision(paper(59950, 500), 1), /risk/i);
  });
});

test("a signal can only receive one initial decision", async () => {
  await withHistory([record], async () => {
    await appendDecision(watch(), 1);
    await assert.rejects(() => appendDecision(skip("OTHER"), 2), /sudah/i);
  });
});

test("journal reads newest first and can filter by action and coin", async () => {
  const eth: SignalRecord = { ...record, key: "ETH-1725000000001", coin: "ETH", signal_closed_at: 1725000000001 };
  await withHistory([record, eth], async () => {
    await appendDecision(watch(), 1);
    await appendDecision({ ...skip("LATE"), signal_key: eth.key }, 2);
    assert.deepEqual((await readDecisions()).map((x) => x.signal.coin), ["ETH", "BTC"]);
    assert.deepEqual((await readDecisions({ action: "WATCH" })).map((x) => x.signal.coin), ["BTC"]);
    assert.deepEqual((await readDecisions({ coin: "eth" })).map((x) => x.signal.coin), ["ETH"]);
  });
});

test("mutation origin must exactly match the request host", () => {
  assert.equal(mutationIsSameOrigin(new Request("https://app.test/api/decisions", { headers: { origin: "https://app.test", host: "app.test" } })), true);
  assert.equal(mutationIsSameOrigin(new Request("https://app.test/api/decisions", { headers: { origin: "https://evil.test", host: "app.test" } })), false);
  assert.equal(mutationIsSameOrigin(new Request("https://app.test/api/decisions", { headers: { host: "app.test" } })), false);
});
